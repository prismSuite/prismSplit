...
export async function processAudio(filePath: string, model: string, outputDir: string, quality: string): Promise<string> {
    if (isTauri()) {
        return await invoke('process_audio', { filePath, model, outputDir, quality });
    } else {
        // Mock for web preview
        return new Promise((resolve) => {
            setTimeout(() => {
                resolve(`(Mock) Procesamiento completado de:\n${filePath}\nusando ${model} a calidad ${quality}.\nResultados en ${outputDir}`);
            }, 3000);
        });
    }
}
