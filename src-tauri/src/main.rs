...
#[tauri::command]
async fn process_audio(state: State<'_, AppState>, file_path: String, model: String, output_dir: String, quality: String) -> Result<String, String> {
    // In a real app, we would look up the stem_separation or gain_staging agent
    let mut registry = state.registry.lock().await;

    // Simulate IPC call to external agent
    println!("Processing audio: {} with model {} at quality {} to {}", file_path, model, quality, output_dir);
    ...
    if let Some(agent_arc) = registry.get_agent("stem_separation") {
        ...
        let result = agent.execute("separate", json!({
            "input": file_path,
            "model": model,
            "output": output_dir,
            "quality": quality
